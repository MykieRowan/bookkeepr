...
if (viaProwlarr && downloadUrl.startsWith('http')) {
  ...
  try {
    const torrentResponse = await axios.get(downloadUrl, {
      ...
      timeout: 10000,
      maxRedirects: 5,  // Follow redirects to get the real torrent
      headers: {
    ...
    console.log(`  Final URL after redirects: ${torrentResponse.request?.res?.responseUrl || 'no redirect'}`);
    ...
    if (announceMatch) {
      ...
      if (announceUrl.includes('passkey=') || announceUrl.includes('authkey=') || announceUrl.includes('torrent_pass=')) {
        ...
        console.log(`  This means Prowlarr didn't inject credentials. Check Prowlarr's MAM indexer settings.`);
      }
    }
